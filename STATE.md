...
- ✅ UX-рефакторинг бота (Сессия 16) — совместимость (фикс + онбординг + двухслойность), профиль (управление подпиской, поддержка), форматирование текстов, таро-подменю (новый расклад «Что мешает»)
...
- 🟡 Отчёт совместимости (HTML/PDF) — шаблон минимальный, требует переработки
  в сессии 17: имена вместо арканов, тип отношений, двухслойность, V2-дизайн
- 🟡 Персонализация таро — все пользователи видят одну карту дня
  (алгоритм по дате, не по матрице). Нужна привязка к центральному аркану матрицы.
...
- **28.05.2026 — Сессия 16** — Claude Sonnet 4.6 (claude.ai)
  - **Диагностика и фикс совместимости** — root cause: `loading_steps` вызывалась
    как список, исправлено на вызов функции `loading_steps()`
  - **Hotfix FSM** — `/start` и `/menu` теперь сбрасывают любое FSM-состояние
    через `await state.clear()` в начале handlers
  - **Совместимость: онбординг** — FSM расширен до 3 шагов:
    имя партнёра → тип отношений (4 кнопки) → дата рождения
  - **Совместимость: анимация** — 4 шага анимации во время генерации
  - **Совместимость: двухслойный подход** — основной текст с именами вместо арканов;
    кнопка «🔍 Как мы это считаем» открывает экспертный технический слой
  - **Совместимость: кнопки** — добавлены «📄 Открыть отчёт» + «⬇️ Скачать PDF»
    для подписчиков; «Новый расклад» → «🔄 Проверить другого человека»;
    «Отправить другу» → «🔗 Поделиться»; убрано двойное уведомление из Celery
  - **Совместимость: цена** — 390 ₽ скрыта у premium-пользователей
  - **Моя матрица** — убран CTA-текст «Хочешь узнать больше?»;
    добавлена кнопка «👁 Посмотреть пример отчёта» для пользователей без покупки
  - **Профиль** — рефакторинг: добавлены «💳 Управление подпиской» (двухшаговая
    отмена с подтверждением) и «🆘 Поддержка» (ссылка на Telegram);
    «📄 Мои отчёты» перенесены в главное меню; «Пример отчёта» убран из главного меню
  - **Форматирование текстов** — создан `bot/utils/formatting.py` с утилитами
    `format_bot_text`, `format_tarot_result`, `format_compatibility_result`;
    все 5 tarot-промптов переписаны полностью: запрет арканов в тексте,
    3 абзаца, блок «Как использовать:», системный промпт обновлён во всех handlers
  - **Таро: подменю «Ещё расклады»** — убраны «Сферы жизни» (дублировали главное меню);
    добавлен новый расклад «🚧 Что мешает» (3 карты: блок / причина / выход)
    с промптом `tarot_blocks.txt`