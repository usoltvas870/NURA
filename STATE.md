...
- Модель: DeepSeek V4 Pro
- Что сделано: Полное обновление `marketing-consent.html` до редакции от 09.07.2026
  - Заменён весь текст документа на новую редакцию «Согласие на получение информационных и рекламных сообщений» (16 разделов)
  - Маркетинговое согласие отделено от согласия на обработку ПД, не является обязательным условием покупки/подписки
  - Сохранён legal-nav со всеми 5 ссылками, дизайн, CSS, шапка, футер
  - Проверены ссылки PWA-профиля — корректны, изменений не требовалось
  - Коммит: `617c705`, запушен
- Блокеры: нет
- Следующие шаги: обновление чекбоксов и бизнес-логики согласия на рассылки (отдельная задача)

## Сессия 83 — 09.07.2026
- Модель: DeepSeek V4 Pro
- Что сделано: Полное обновление `privacy.html` до редакции от 09.07.2026
  - Заменён весь текст документа на новую редакцию «Политика обработки персональных данных» (19 разделов)
  - Добавлены новые разделы: Принципы обработки, Данные VK ID, Особенности AI-функций, Данные третьих лиц, Отзыв согласия, Обезличенные данные
  - Добавлены две HTML-таблицы: цели обработки (5 колонок × 12 строк) и передача данных третьим лицам (3 колонки × 8 строк)
  - Добавлен CSS для `.legal-wrap` + `.legal-table` с горизонтальным скроллом для мобильных
  - Обновлены meta description и дата редакции (04.07 → 09.07.2026)
  - legal-nav сохранён без изменений (5 ссылок)
- Файлы: `privacy.html`
- Блокеры: нет
- Следующие шаги: деплой на VPS, визуальная проверка таблиц на мобильном
...
## Full Matrix Telegram delivery and repeated access — PASS, 27.07.2026

- A completed paid full Report owns one canonical PDF artifact and one automatic `FullReportTelegramDelivery`. Automatic completion and reconciliation use the same idempotent row; manual access from My Reports creates a separate delivery for the same Report/artifact without creating AI/Matrix work, a Report, generation job, Order, PaymentAttempt, or PaymentEvent.
- Delivery uses atomic claim, `attempt_count` fencing, fresh-claim protection, stale recovery, typed retryable/terminal failures, and durable Telegram message/file receipts. The latest completed delivery is the `file_id` source of truth, including `NULL`: a reusable ID avoids artifact upload, while only typed `invalid_file_id` permits one validated artifact fallback and persists the replacement ID.
- Telegram send runs after the claim transaction commits and before a separate fenced terminal transaction. The normal flow is at-most-one active worker and effectively-once; Telegram has no application idempotency key, so the acknowledged send-before-commit crash window can cause a duplicate after stale recovery and absolute exactly-once is not claimed.
- Refund/account-deletion entitlement is re-read immediately before each Telegram transport call without holding a database lock during external I/O. A narrow revocation-check-to-provider-call TOCTOU window therefore remains; closing it would require a cross-system revocation/send lease and is deferred rather than violating the accepted no-DB-transaction-during-Telegram boundary.
- Existing report reconciliation now repairs missing automatic deliveries, atomically claims queued/retryable/stale deliveries, publishes only for the claim winner, ignores fresh/completed/non-retryable rows, and cancels refunded rows. Mini-report reconciliation and the separate `TelegramReportDelivery` lifecycle remain unchanged.
- Refund is rechecked before every Telegram transport call, including artifact fallback, and blocks future manual access. Account deletion removes User, Report, canonical artifact bytes, active/historical full-delivery rows, and Telegram receipt metadata while retaining anonymized financial Order/PaymentAttempt/PaymentEvent records; stale terminal updates cannot restore deleted rows.
- Migration `f9a0b1c2d3e4` (parent `e8f9a0b1c2d3`) passed disposable PostgreSQL 16 blank/parent upgrade, exact schema/constraint/index/FK inspection, downgrade/re-upgrade, preservation, and eight-worker create/claim/stale/fencing/manual/file-ID/refund/deletion/reconciliation races. Disposable containers were removed.
- Targeted acceptance: 92 passed. Full safe Python suite: 12 file-shards, 1011 passed, 17 skipped, one approved Tarot asset deselection, failed=0, 60/60 included test files covered exactly once, missing=0, duplicates=0. Exact worktree allowlist, Ruff, diff, and Alembic checks pass.
- Graphify 0.9.7: hooks are disabled, `graphify check-update .` passed with no generated diff, and Graphify update is not required. Production fiscal configuration remains fail-closed and is not changed by this stage.
- FULL MATRIX TELEGRAM DELIVERY AND REPEATED ACCESS is accepted. After a separate commit, the next stage is TELEGRAM-FIRST END-TO-END SANDBOX ACCEPTANCE AND LAUNCH READINESS.
