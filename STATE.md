...
1. 🔴 **Сгенерировать реальный отчёт** → использовать как `sample-report.html` на лендинге
2. 🟡 **Фикс why-аккордеон** — стиль кода с `//` в блоках S3–S13 (сейчас текст слитно)
3. 🟡 **Диагностика: Карта здоровья + Психоблоки** — проверка данных в БД, пустые секции в старых отчётах
4. 🟡 **Деплой sample-report.html** на лендинг (`/var/www/nura-ai.ru/index.html` секция `#report-sample`)
5. 🔴 **YooKassa** — ждём владельца (shop_id + secret_key)
