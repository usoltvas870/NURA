...
Тестируем: tarot, start, profile, payment, compatibility, insights.
...
# =====================================================================
# INSIGHTS HANDLER TESTS (из test_handlers.py)
# =====================================================================


class TestInsightsHandler:
    """Тесты show_insight — повторяет логику из test_handlers.py."""

    @pytest.mark.asyncio
    async def test_free_user_shows_static(self):
        """Свободный пользователь видит статичный инсайт."""
        mock_user = MagicMock()
        mock_user.main_archetype = "Маг"
        mock_user.main_archetype_number = 1
    ...
    async def test_subscriber_shows_ai(self):
        ...
        mock_insight = DailyInsightResult(
            insight="AI insight for premium user",
            focus_area="карьера",
        )
        ...
        with (
            patch("bot.handlers.insights.UserRepository") as MockRepo,
            patch(
                "bot.handlers.insights.AIService.generate_daily_insight",
    ...
    async def test_subscriber_cached_insight(self):
        """Подписчик видит кэшированный инсайт."""
        mock_user = MagicMock()
        mock_user.main_archetype = "Маг"
        mock_user.main_archetype_number = 1
        mock_user.subscription_status = "premium"
    ...
    async def test_free_user_exhausted_shows_subscription(self):
        """Free-пользователь без лимитов — предложение подписки."""
        mock_user = MagicMock()
        mock_user.main_archetype = "Маг"
        mock_user.main_archetype_number = 1
        mock_user.subscription_status = "free"
