...
from core.models import Order, PaymentAttempt, PaymentEvent, Report, ReportGenerationJob, User
from core.services.account_deletion import AccountDeletionService


async def _seed(factory: async_sessionmaker[AsyncSession], user: User) -> tuple[uuid.UUID, uuid.UUID, uuid.UUID]:
    now = datetime.now(timezone.utc)
    async with factory() as session:
        report = Report(id=uuid.uuid4(), user_id=user.id, report_type="full", token="retention-report", matrix_data={"secret": "remove"})
        session.add(report)
        await session.flush()
...
async def test_account_deletion_rolls_back_retention_mutations_on_failure(db_engine, test_user, monkeypatch):
    factory = async_sessionmaker(db_engine, class_=AsyncSession, expire_on_commit=False)
    order_id, _, _ = await _seed(factory, test_user)
