...
```
MatrixStates:
  - waiting_birth_date        # Ожидание даты для матрицы

CompatibilityStates:
  - waiting_first_date        # Ожидание первой даты
  - waiting_second_date       # Ожидание второй даты
...
**Важно:** `ChatStates.idle` — это состояние, в котором бот находится по умолчанию. Когда пользователь вводит текст вне FSM-состояний, бот проверяет `ChatStates.chatting`. Если пользователь не в чате — текст игнорируется (или предлагает меню).
...
**callback_data:** `calculate_matrix`, `compatibility`, `insights`, `chat_with_nura`, `profile`
...
[Бот запрашивает дату рождения]
    ↓  state = MatrixStates.waiting_birth_date
[Пользователь вводит дату]
    ↓
[Валидация]
    ├── Неверный формат → ошибка → повторный ввод
    ├── Невозможная дата → ошибка → повторный ввод
    └── OK
        ↓  state.clear()
[Loading-анимация (4 шага, 1 сообщение, редактируется)]
    ↓
...
### 3.5. Loading-анимация

Реализация: одно сообщение, которое 4 раза редактируется с паузами 1-1.5 сек.
...
```python
async def loading_animation(message: Message, birth_date: str):
    steps = [
        "✶ Считаю твою матрицу...\n\nРасшифровываю 22 аркана из чисел твоего рождения",
        "◈ Анализирую архетипы...\n\nСопоставляю энергии с твоими личными сценариями",
        "☯ Синхронизирую с энергиями...\n\nСобираю целостную картину твоей матрицы",
...
Каждый блок приходит из AI-ответа. Формат ответа от `generate_mini_report`:

```python
{
    "main_archetype": {"name": "Императрица", "number": 3, "text": "..."},
    "core_strength": {"name": "Изобилие", "text": "..."},
    "emotional_conflict": {"name": "Жертвенность", "text": "..."},
    "relationship_pattern": {"name": "Спасатель", "text": "..."},
    "financial_block": {"name": "Стыд за деньги", "text": "..."},
}
```
...
[Объяснение фичи]
    ↓
[Запрос первой даты]
    ↓  state = CompatibilityStates.waiting_first_date
[Ввод первой даты]
    ↓  валидация
[Запрос второй даты]
    ↓  state = CompatibilityStates.waiting_second_date
[Ввод второй даты]
    ↓  валидация
[Loading-анимация]
    ↓
[Мини-разбор совместимости (2-3 бесплатных блока)]
    ↓
[CTA: Полный разбор совместимости — в составе полной матрицы 590 ₽ или подписки 390 ₽/мес]
```
...
### 4.6. Ошибки валидации (те же, что в матрице + одна дополнительная)
...
```
┌─────────────────────────────────────────────┐
│ 🔮 Полная матрица 590 ₽ — полный разбор      │
├─────────────────────────────────────────────┤
│ 💎 Подписка 390 ₽/мес — ежедневные инсайты   │
├─────────────────────────────────────────────┤
│ 🏠 В меню                                   │
└─────────────────────────────────────────────┘
```
...
callback: chat_with_nura
    ↓
[Проверка доступа: подписка или полный отчёт есть?]
    ├── НЕТ → пейволл
    └── ДА
        ↓
[Приветствие]
...
**callback_data:** `pay_full_report:{token}`, `buy_subscription`, `back_to_profile`

### 7.3. Приветствие при входе в чат

Использует имя пользователя + имя архетипа:
...
**System prompt (загружается из `core/prompts/chat_system.txt`):**
...
### 7.7. Обработка сообщений вне чата
...
```
[Пользователь нажимает 🔮 Полный разбор за 590 ₽]
    ↓
...
```
┌─────────────────────────────┐
│ 💳 Оплатить 590 ₽           │  ← InlineKeyboardButton с URL
├─────────────────────────────┤
│ 🏠 В меню                   │
└─────────────────────────────┘
```
...
В нём:
• Разбор всех 22 арканов твоей матрицы
...
| Расчёт матрицы | ✅ | ✅ | ✅ |
| Мини-разбор (5 блоков) | ✅ | ✅ | ✅ |
| Полный AI-отчёт (матрица) | ❌ | ✅ (1 шт) | ✅ (1 шт при покупке) |
| Совместимость (бесплатные блоки) | ✅ | ✅ | ✅ |
| Полный разбор совместимости | ❌ | ✅ (в составе матрицы) | ❌ |
| Чат с NURA | ❌ | ✅ | ✅ |
| Ежедневные инсайты | ❌ | ❌ | ✅ |
...
ARCANA = {
    0: {"name": "Шут", "emoji": "🌀"},
    1: {"name": "Маг", "emoji": "✨"},
    2: {"name": "Верховная Жрица", "emoji": "🌙"},
    3: {"name": "Императрица", "emoji": "🌷"},
    4: {"name": "Император", "emoji": "👑"},
    5: {"name": "Иерофант", "emoji": "📜"},
    6: {"name": "Влюблённые", "emoji": "💞"},
    7: {"name": "Колесница", "emoji": "⚔️"},
    8: {"name": "Сила", "emoji": "🦁"},
    9: {"name": "Отшельник", "emoji": "🏮"},
    10: {"name": "Колесо Фортуны", "emoji": "🎰"},
    11: {"name": "Справедливость", "emoji": "⚖️"},
    12: {"name": "Повешенный", "emoji": "🌀"},
    13: {"name": "Смерть", "emoji": "♠️"},
    14: {"name": "Умеренность", "emoji": "⚗️"},
    15: {"name": "Дьявол", "emoji": "⛓️"},
    16: {"name": "Башня", "emoji": "🗼"},
    17: {"name": "Звезда", "emoji": "⭐"},
    18: {"name": "Луна", "emoji": "🌑"},
    19: {"name": "Солнце", "emoji": "☀️"},
    20: {"name": "Суд", "emoji": "📯"},
    21: {"name": "Мир", "emoji": "🌍"},
}
```
