...
def main():
    ...
    name = f"nura_scale_{uuid.uuid4().hex[:6]}"
    ...
    if dur_sec <= 0:
        dur_sec = 10.0
    n_segs = max(1, min(8, int(dur_sec * 1_000_000 / SEG_DUR)))
    print(f"Video: {dur_sec:.1f}s, segments: {n_segs}")
    ...
    segments = []
    labels = [
        "Оригинал", "Zoom In 1.3x", "Zoom Out 0.7x",
        "Scale X 1.5x", "Scale Y 1.5x",
        "Keyframe Zoom In", "Keyframe Zoom Out",
        "Keyframe Pulse",
    ]
    ...
    for i in range(n_segs):
        ...
        if i == 0:
            ...
            seg["clip"]["scale"]["x"] = 0.7
            seg["clip"]["scale"]["y"] = 0.7
        elif i == 3:
            seg["clip"]["scale"]["x"] = 1.5
        elif i == 4:
            seg["clip"]["scale"]["y"] = 1.5
        elif i == 5:
            ...
            seg["common_keyframes"] = [
                {"id": uid_hex(), "keyframe_list": [
                    {"curveType": "Line", "graphID": "", "left_control": {"x": 0, "y": 0}, "right_control": {"x": 0, "y": 0}, "id": uid_hex(), "time_offset": 0, "values": [1]},
                    {"curveType": "Line", "graphID": "", "left_control": {"x": 0, "y": 0}, "right_control": {"x": 0, "y": 0}, "id": uid_hex(), "time_offset": SEG_DUR, "values": [0.6]},
                ], "material_id": "", "property_type": "UNIFORM_SCALE"},
            ]
        elif i == 7:
            mid = SEG_DUR // 2
            seg["common_keyframes"] = [
                {"id": uid_hex(), "keyframe_list": [
                    {"curveType": "Line", "graphID": "", "left_control": {"x": 0, "y": 0}, "right_control": {"x": 0, "y": 0}, "id": uid_hex(), "time_offset": 0, "values": [1]},
                    {"curveType": "Line", "graphID": "", "left_control": {"x": 0, "y": 0}, "right_control": {"x": 0, "y": 0}, "id": uid_hex(), "time_offset": mid, "values": [1.4]},
                    {"curveType": "Line", "graphID": "", "left_control": {"x": 0, "y": 0}, "right_control": {"x": 0, "y": 0}, "id": uid_hex(), "time_offset": SEG_DUR, "values": [1]},
                ], "material_id": "", "property_type": "UNIFORM_SCALE"},
            ]
    ...
    for i, seg in enumerate(segments):
        start = i * SEG_DUR
        label = labels[i] if i < len(labels) else f"Seg {i+1}"
        tmat_id = uid()
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python scripts/build_scale_demo.py <video.mp4>")
        sys.exit(1)
    main()
